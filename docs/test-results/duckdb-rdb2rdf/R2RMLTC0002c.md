...
Traceback (most recent call last):
  ...
  File "/opt/miniconda3/envs/rdf/lib/python3.9/site-packages/duckdb_engine/__init__.py", line 144, in execute
    self.__c.execute(statement, parameters)
sqlalchemy.exc.ProgrammingError: (duckdb.BinderException) Binder Error: Table "Student" does not have a column named "IDs"
LINE 1: ... AS s, '<http://example.com/id>' AS p, "Student"."IDs" AS o, NULL AS g 
                                                  ^
[SQL: SELECT ? || replace(replace(replace(replace(replace(replace(CAST("Student"."ID" AS VARCHAR), ?, ?), ?, ?), ?, ?), ?, ?), ?, ?), ?, ?) || ? || replace(replace(replace(replace(replace(replace(CAST("Student"."Name" AS VARCHAR), ?, ?), ?, ?), ?, ?), ?, ?), ?, ?), ?, ?) || ? AS s, '<http://example.com/id>' AS p, "Student"."IDs" AS o, NULL AS g 
FROM "Student"]
