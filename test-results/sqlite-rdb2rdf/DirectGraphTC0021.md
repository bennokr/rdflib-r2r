...
SELECT anon_1.s AS s,
       ...
       anon_1.o AS o
FROM
  (SELECT CAST('<' AS VARCHAR) || CAST('Target/PK=' AS VARCHAR) || replace(replace(replace(replace(replace(replace(CAST("Target"."PK" AS VARCHAR), ' ', '%20'), '/', '%2F'), '(', '%28'), ')', '%29'), ',', '%2C'), ':', '%3A') || CAST('>' AS VARCHAR) AS s,
   ...
   WHERE "Source"."attrB" = "Target_ref"."key2attr1"
     AND "Source"."attrA" = "Target_ref"."key2attr2") AS anon_1
```
